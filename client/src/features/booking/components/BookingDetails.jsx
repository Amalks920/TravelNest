        ...
        </div>
{
   ...
   return  <>
        <div className="col-span-1 flex justify-center items-center p-7 border-t-2">
           <img src={IMAGE_BASE_URL+hotelImages[0]} alt="" width={150} height={150} className="rounded-md md:h-[80px] h-[50px]" />
        </div>
        <div className="col-span-1 flex flex-col justify-center items-left gap-3 border-t-2">
           <h2 className="font-normal md:text-[1.1rem] sm:text-[0.8rem] text-[0.6rem] ">{hotelName}</h2> 
           <h2 className="md:text-[0.6rem] text-[0.5rem]">{ format(checkIn, 'yyyy-MM-dd HH:mm:ss', { timeZone: 'Asia/Kolkata' })} - { format(checkOut, 'yyyy-MM-dd HH:mm:ss', { timeZone: 'Asia/Kolkata' })}</h2>
        </div>
        <div className="col-span-1 flex flex-col justify-center items-center gap-2 border-t-2">
            <h2 className="font-normal sm:text-sm text-[0.7rem]">{status}</h2>
            <h2 className="font-bold sm:text-sm text-[0.7rem]">₹{totalAmount}</h2>
            <Link to={`/single-booking-details/${_id}`} className="text-red-500 sm:text-[0.6rem] text-[0.4rem] ms-10 cursor-pointer">View Details?</Link>
        </div>
